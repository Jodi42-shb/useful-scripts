...
def find_first_image(folder_path):
    ...
    try:
        ...
        logging.warning(f"No valid image found for cover in {folder_path}")
        return None
    except Exception as e:
        logging.error(f"Error finding cover image in {folder_path}: {e}")
        return None
...
def organize_manga_inplace(source_dir):
    ...
    for series_folder in os.listdir(source_dir):
        ...
        try:
            # Find cover image before moving files
            cover_image = find_first_image(series_path)
            cover_copied = False
            ...
            # Copy cover image (1.ext) to series folder
            if cover_image:
                cover_ext = os.path.splitext(cover_image)[1]
                cover_dst = series_path / f"cover{cover_ext}"
                cover_src = chapter_path / os.path.basename(cover_image)
                if cover_src.exists() and not cover_dst.exists():
                    try:
                        shutil.copy2(cover_src, cover_dst)
                        logging.info(f"Copied cover {cover_src} to {cover_dst}")
                        cover_copied = True
                    except Exception as e:
                        logging.error(f"Error copying cover {cover_src} to {cover_dst}: {e}")
                else:
                    logging.warning(f"Cover source {cover_src} not found or {cover_dst} already exists")

            if not cover_copied:
                logging.warning(f"No cover copied for {series_title}")
