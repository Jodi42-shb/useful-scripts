...
def organize_manga_inplace(source_dir):
    ...
    for series_folder in os.listdir(source_dir):
        ...
        try:
            ...
            if cover_image:
                ...
                if not cover_dst.exists():
                    try:
                        ...
                        logging.error(
                            f"Error copying cover {cover_image} to {cover_dst}: {e}"
                        )
                else:
            ...
            print(
                f"Error processing {series_title}. Check reorganize_log.txt for details."
            )
...
def main():
    ...
    source_dir = input(
        "Enter the OneDrive directory containing manga folders (e.g., C:\\Users\\YourName\\OneDrive\\manga_folders): "
    ).strip()
